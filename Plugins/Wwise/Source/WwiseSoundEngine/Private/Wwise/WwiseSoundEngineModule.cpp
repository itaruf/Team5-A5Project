...
void FWwiseSoundEngineModule::StartupModule()
{
	...
	auto* ModuleInterface = ModuleManager.LoadModule("WwiseSoundEngine_" AK_WWISE_SOUNDENGINE_VERSION);
	if (UNLIKELY(!ModuleInterface))
	{
		UE_LOG(LogWwiseSoundEngine, Error, TEXT("Could not load Sound Engine module WwiseSoundEngine_") TEXT(AK_WWISE_SOUNDENGINE_VERSION));
		return;
	}

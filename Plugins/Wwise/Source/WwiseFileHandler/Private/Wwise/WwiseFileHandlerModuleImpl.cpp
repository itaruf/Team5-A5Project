...
void FWwiseFileHandlerModule::ShutdownModule()
{
	...
	{
		UE_LOG(LogWwiseFileHandler, Display, TEXT("Shutting down default SoundBank Manager."));
		SoundBankManager.Reset();
	}
	if (ExternalSourceManager.IsValid())
	{
		UE_LOG(LogWwiseFileHandler, Display, TEXT("Shutting down default External Source Manager."));
		ExternalSourceManager.Reset();
	}
	if (MediaManager.IsValid())
	{
		UE_LOG(LogWwiseFileHandler, Display, TEXT("Shutting down default Media Manager."));
		MediaManager.Reset();
	}
	Lock.WriteUnlock();
